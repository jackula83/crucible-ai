...
class Harness {
  ...
  static renderedFailure(
    ...
    return new Reporter().renderFailed(Harness.context(verdicts, errors), cause);
  }
}

describe('Reporter', () => {
  it('renders the claim, the response, and the judge reasoning for a false verdict', () => {
    const message = Harness.renderedFailure([
      Harness.falseVerdict({
  ...
  it('truncates a long response to 500 characters and marks the cut', () => {
    ...
    const message = Harness.renderedFailure([Harness.falseVerdict({ response })]);
  ...
  it('renders only the lowest-seq false verdict when several exist', () => {
    const message = Harness.renderedFailure([
      Harness.falseVerdict({ claim: 'later claim', reasoning: 'later reasoning', seq: 4 }),
      Harness.falseVerdict({ claim: 'first claim', reasoning: 'first reasoning', seq: 1 }),
    ]);
  ...
  it('skips true verdicts when picking the failing one', () => {
    const message = Harness.renderedFailure([
      Harness.falseVerdict({ claim: 'passing claim', verdict: true, seq: 0 }),
      Harness.falseVerdict({ claim: 'failing claim', reasoning: 'the mismatch', seq: 2 }),
    ]);
  ...
  it('picks the first failure across verdicts and error records by seq', () => {
    const message = Harness.renderedFailure(
      [Harness.falseVerdict({ claim: 'later claim', seq: 3 })],
      new Error('terminal'),
  ...
  it('falls back to the original cause when no false verdict was recorded', () => {
    ...
    const message = Harness.renderedFailure([], cause);
  ...
  it('renders a non-Error object rejection legibly, never as [object Object]', () => {
    const message = Harness.renderedFailure([], { code: 'ECONNRESET' });
  ...
  it('survives a cause whose stringification throws', () => {
    ...
    const message = Harness.renderedFailure([], hostile);
  ...
  it('renders an errored run as errored, not failed, and names the infrastructure cause', () => {
    ...
    const message = new Reporter().renderErrored(Harness.context(), cause);
  ...
  it('keeps failed-run wording free of the errored-run marker', () => {
    const failed = Harness.renderedFailure([Harness.falseVerdict()]);
